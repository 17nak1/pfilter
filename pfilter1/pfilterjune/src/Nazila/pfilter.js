...
pfilter.run = function(input){
  ...
  let stateSaved = []
  let temp 
  let timeCountData = 0, ws ,w , vsq, sumsq, ess, loglik = 0, lik, fail 
  ...
  for (indexk = 0; indexk < ntimesk; indexk++){
    ...
    connect to pfilterComputation = function (x, params, p, runPredMean, runPredVar, runFilterMean, trackancestry, doparRS, weights, tol)
    // Normalize
      // let sumOfWeights = 0
      // for (let i = 0; i < Np; i++) {
