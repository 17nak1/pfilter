 ...
 *
 * @references         pfilter.R by Aaron A. King. https://github.com/kingaa/pomp/blob/abc552b335319bd36b21d3313305c89e97f48f68/R/pfilter.R
                       A Tutorial on Particle Filters for Online Nonlinear, Non-Gaussian Bayesian Tracking. M. S. Arulampalam, S. Maskell, N. Gordon, & T. Clapp
...
pfilter.run = function(input){
  ...
  // Creat the initial array for requested output
  if (doPredictionMean) {
  ...
  // 'state' is the initial sets of states.
  // 'particles' =  store states during times.
  // 
  ...
  // Time loop t0:last time in datacases
  for (k = t0; k <= Number(dataCases[timeLen - 2][0]) + deltaT / 3 ; k += deltaT){
    ...
    // Start sampling for all Np points and let H = 0 and then simulate new states to replace in particles.
    if ( k > timeDataStart) {
    ...
    temp = simulator.simulate (Np, temp, k, deltaT, dt, interpolPop, interpolBirth, params, timeDataStart,Number(dataCases[timeCountData + 1][0]))
    ...
    for (np = 0; np < Np; np++){ 
      ...
      /* Weight
       * based on the value of H and data in datacases dmeasure calculte the loglik for each Np points.
       */
      if (k >= timeDataStart){
        if (stateSaved) {
    ...
    if (k >= timeDataStart){
      ...
      // Check the weights to see if the particle is lost. Compute sum and sum of squares
      w = 0, ws = 0, nlost = 0
      for (let i = 0; i < Np; i++) {
        if (weights[i] > toler) {
          w += weights[i]
