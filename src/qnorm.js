...
exports.qnorm5 = function (p, mu, sigma, lower_tail, log_p) {

  if (isNaN(p) || isNaN(mu) || isNaN(sigma))
	  return p + mu + sigma;

  R_Q_P01_boundaries(p, -Infinity, Infinity, lower_tail, log_p);
...
const R_Q_P01_boundaries = function(p, _LEFT_, _RIGHT_, lower_tail, log_p )	{
  if (log_p) {				
    if(p > 0)				
      return NaN;			
    if(p == 0) /* upper bound*/		
