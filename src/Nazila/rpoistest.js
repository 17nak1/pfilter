...
let mucache = []
mucache[mucache.length %100] = process.memoryUsage()

let lastMU = process.memoryUsage()

    let fact = [1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880];
rpoistest.rpoisOne = function (mu,ttt) {
    let thisMU = process.memoryUsage()
    if( thisMU.heapTotal !== lastMU.heapTotal) console.log(ttt, thisMU.heapTotal, lastMU.heapTotal, (thisMU.heapTotal- lastMU.heapTotal)/1E6)
    // console.log((thisMU.heapTotal === lastMU.heapTotal) ? 1 : thisMU.heapTotal - lastMU.heapTotal, new Error("XXX").stack.split('\n')[2])
    // console.log(thisMU.heapTotal)
    lastMU = thisMU
    let l = 0;
    let m = 0;
    let pp = new Array(36);
